...
@compute @workgroup_size(128) fn deposit(@builtin(global_invocation_id) id:vec3u){let i=id.x;if(i>=u32(params.counts.x)||params.random.y<.5){return;}let p=voxels[i].position.xyz;let g=groundSample(p.xz);let contact=clamp(1.-(p.y-g.x-.145)/.3,0.,1.);if(contact<=0.){return;}let f=field[cellIndex(cellAt(p))];let weight=bodyMeta[i].physical.x*max(0.,-f.gravityDrag.y)*contact*.5;let uv=clamp((p.xz+20.)/.625,vec2f(0),vec2f(63.9999));let c=vec2u(floor(uv));let t=fract(uv);let k=c.x+c.y*65u;
